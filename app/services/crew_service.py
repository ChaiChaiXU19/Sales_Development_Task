    ...
    return {
        "diagnosis": TaskPromptSpec(
            description=(
                ...
                "3. 只做诊断，不允许提出策略、建议、应对措施，也不要偷渡动作话术。\n"
                "4. 结合权重，明确当前最薄弱的要素和最危险的卡点。\n"
                "5. 允许用六要素做内部判断，但对外禁止显式暴露方法论框架。"
            ),
            expected_output=(
                "一份口语化的诊断报告，按六要素说明事实、缺口、风险和卡点，不包含任何对策、"
                "术语标签或方法论名称。"
            ),
        ),
        "strategy": TaskPromptSpec(
            description=(
                ...
                "3. 只能依据六要素策略库和检索结果出招，不能凭空发挥。\n"
                "4. 动作必须可执行，避免抽象口号，所有术语都要翻译成客户沟通口语。\n"
        ...
        "challenge": TaskPromptSpec(
            description=(
                ...
                "4. 每条挑战意见都要明确指出原动作的生硬度问题，并补一条柔和切入点、替代表达和 How to say。\n"
                "5. 优先引用检索到的失败教训、产品能力边界和常见交付风险；若检索为空，必须明确这一点。"
            ),
            expected_output=(
                "一份口语化的蓝军挑战报告，逐条指出原动作的问题、生硬度风险、竞争反制、必须修正点，"
                "并给出柔和切入点、替代表达和 How to say。"
            ),
        ),
        "closer": TaskPromptSpec(
            description=(
                ...
                "3. 每条动作必须固定格式：Who: ... | When: ... | Do What: ... | How to say: ...\n"
                "4. Who 必须明确是销售本人、合作伙伴、技术人员、高层或其他角色。\n"
                "5. When 必须明确时间节点、最晚时间或阶段窗口。\n"
                "6. Do What 必须是可量化、可执行的动作，并已经吸收挑战阶段给出的柔和切入点。\n"
                "7. How to say 必须是一句可以直接复制给客户或关键人的实战话术。\n"
                "8. 优先使用检索到的成功打法、失败教训和产品能力边界来约束最终动作。\n"
                "9. 禁止输出前言、结论、分析、解释，只输出最终动作清单。"
            ),
            expected_output=(
                "仅输出 3-5 条最终动作清单，每条使用 Who / When / Do What / How to say 格式，"
                "语言口语化，不出现方法论标签，不附加任何分析说明。"
            ),
        ),
    }
